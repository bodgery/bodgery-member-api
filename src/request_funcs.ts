...
export function set_db(new_db: db_impl.DB, new_typeorm_connection) {
    db = new_db;
...
export function put_member(req, res) {
    ...
    db.add_member(
        ...
        member_id => {
            ...
            res.status(201).send({id: member_id}).end();
        },
...
export function get_member(req, res) {
    ...
    try {
        valid.validate(req.params, [valid.isUUID('member_id')]);
    } catch (err) {
        handle_generic_validation_error(logger, res, err);
        return;
    }
...
export function put_member_address(req, res) {
    ...
    try {
        valid.validate(req.params, [valid.isUUID('member_id')]);
        valid.validate(body, [valid.isUSAddress()]);
    } catch (err) {
        handle_generic_validation_error(logger, res, err);
        return;
    }
...
export function put_member_is_active(req, res) {
    ...
    try {
        valid.validate(req.params, [valid.isUUID('member_id')]);
        valid.validate(req.body, [valid.isBoolean('is_active')]);
    } catch (err) {
        handle_generic_validation_error(logger, res, err);
        return;
    }
    ...
    logger.info(
        'Setting member status to ' + status + ' in Wild Apricot: ' + member_id,
    );
        ...
        wa_id => {
            let success_callback = () => {
                ...
                db.set_member_is_active(
                    ...
                    () => {
                        logger.info(
                            'Member status now ' + status + ': ' + member_id,
                        );
                        res.sendStatus(200);
                    },
                    get_member_id_not_found_error(logger, res, member_id),
            ...
            let error_callback = (err: Error) => {
                logger.error(
                    'Error calling Wild Apricot to set active: ' +
                        err.toString(),
                );
                res.sendStatus(500).end();
            };
...
export function get_member_is_active(req, res) {
    ...
    db.get_member_is_active(
        ...
        (is_active: boolean) => {
            ...
            res.status(200).send(is_active).end();
        },
        get_member_id_not_found_error(logger, res, member_id),
        get_generic_db_error(logger, res),
    );
...
export function put_member_rfid(req, res) {
    ...
    try {
        valid.validate(req.params, [valid.isUUID('member_id')]);
        valid.validate(req.body, [valid.isInteger('rfid')]);
    } catch (err) {
        handle_generic_validation_error(logger, res, err);
        return;
    }
...
export function get_member_rfid(req, res) {
    let logger = req.ctx.logger;
    try {
        valid.validate(req.params, [valid.isInteger('rfid')]);
    } catch (err) {
        handle_generic_validation_error(logger, res, err);
        return;
    }
...
export function put_member_wildapricot(req, res) {
    ...
    try {
        valid.validate(req.params, [valid.isUUID('member_id')]);
        valid.validate(body, [valid.isInteger('wild_apricot_id')]);
    } catch (err) {
        handle_generic_validation_error(logger, res, err);
        return;
    }
            ...
            fetch_google_auth(
                ...
                auth => {
                    ...
                    Promise.all(promises)
                        ...
                        .catch(err => {
                            logger.error(
                                'Error signing up for Google Groups:' +
                                    ' ' +
                                    err,
                            );
                            res.sendStatus(500).end();
...
export function logout_user(req, res) {
    ...
    if (is_logged_in) {
        ...
        res.status(403).render('home', {
            errors: ["Not logged in, so you can't log out"],
        });
    }
}
...
export function rfid_log(req, res) {
    ...
    db.get_rfid_log(
        ...
        logs => {
            logger.info(
                'Fetched ' + logs.length + ' logs starting at ' + offset,
            );
            ...
            let render = tmpl_view('entry-log', render_args, [], 200);
            render(req, res);
        },
        get_generic_db_error(logger, res),
    );
...
function render_tokens(req, res) {
    ...
    db_manager
        ...
        .then(db_user => {
            return db_manager.getRepository(db_access_token.access_token).find({
                user: db_user,
            });
        })
        .then(tokens => {
...
export function add_token(req, res) {
    let logger = req.ctx.logger;
    try {
        valid.validate(req.body, [valid.isWords('name')]);
    } catch (err) {
        handle_generic_validation_error(logger, res, err);
        return;
    }
...
export function is_user_logged_in(req, res) {
    ...
    logger.info(
        'Username [' +
            username +
            '], checking logged in setting' +
            ', which is ' +
            is_logged_in,
    );

    res.status(is_logged_in ? 200 : 403)
...
export function get_members_pending(req, res) {
    ...
    let success_callback = (members: Array<wa_api.WAMember>) => {
        ...
        res.status(200).send(members).end();
    };

    let error_callback = (err: Error) => {
        logger.error(
            'Could not fetch pending members from Wild Apricot: ' +
                err.toString(),
        );
        res.status(500).send('Error fetching data from Wild Apricot').end();
    };

    wa.fetch_pending_members(success_callback, error_callback);
}

export function member_signup(req, res) {
    let logger = req.ctx.logger;
    try {
        valid.validate(req.query, [valid.isInteger('wa_id')]);
    } catch (err) {
        handle_generic_validation_error(logger, res, err);
        return;
    }
    ...
    wa.fetch_member_data(
        ...
        member => {
            ...
            size_limit = sprintf.sprintf('%0.1f MB', size_limit / 1024 / 1024);
        ...
        err => {
            logger.info(
                'Error fetching WA info for member <' +
                    wa_id +
                    '>: ' +
                    err.toString(),
            );
...
export function members_active(req, res) {
    ...
    db.get_members(
        ...
        members => {
            logger.info(
                'Fetched ' + members.length + ' members starting at ' + offset,
            );
            ...
            let render = tmpl_view('members-active', render_args, [], 200);
            render(req, res);
        },
        get_generic_db_error(logger, res),
    );
...
export function member_info(req, res) {
    ...
    try {
        valid.validate(params, [valid.isUUID('member_id')]);
    } catch (err) {
        handle_generic_validation_error(logger, res, err);
        return;
    }
    ...
    db.get_member(
        ...
        member => {
            ...
            let render = tmpl_view('member', render_args, [], 200);
            render(req, res);
        },
                ...
                google_client => {
                    ...
                    sender.init(() => {
                        sender.send_new_member_signup({
                            ...
                            from_name:
                                req.ctx.conf[
                                    'email_new_member_signup_from_name'
                                ],
                            from_email:
                                req.ctx.conf[
                                    'email_new_member_signup_from_email'
                                ],
                            success_callback: () => {
                            ...
                            error_callback: (err: Error) => {
                                logger.error(
                                    'Error sending new member email: ' +
                                        err.toString(),
                                );
                                res.sendStatus(500).end();
                            },
...
export function post_group_member_signup_email(req, res) {
            ...
            google_client => {
                ...
                sender.init(() => {
                    sender.send_new_group_member_signup({
                        to_name:
                            req.ctx.conf[
                                'email_group_new_member_signup_to_name'
                            ],
                        to_email:
                            req.ctx.conf[
                                'email_group_new_member_signup_to_email'
                            ],
                        from_name:
                            req.ctx.conf['email_new_member_signup_from_name'],
                        from_email:
                            req.ctx.conf['email_new_member_signup_from_email'],
    ...
    db.get_member_wild_apricot(
        member_id,
        wild_apricot_id => {
            req.ctx.wa.fetch_member_answers(
                wild_apricot_id,
                ...
                (err: Error) => {
                    logger.error(
                        'Error fetching member answers from' +
                            ' Wild Apricot: ' +
                            err.toString(),
                    );
                    res.sendStatus(500).end();
...
export function put_member_photo(req, res) {
    ...
    let promises = [
        new Promise((resolve, reject) =>
            fs.writeFile(path, decoded_photo, err => {
                if (err) reject(err);
                else resolve();
            }),
        ),
...
export function tmpl_view(
    ...
    return (req, res) => {
        ...
        res.status(status_code).render(view, args);
    };
}
