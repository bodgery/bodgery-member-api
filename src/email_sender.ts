...
export class Email {
    ...
    constructor(args: {auth: oauth.OAuth2Client}) {
        this.auth = args.auth;
    }
    ...
    public send_new_group_member_signup(args: {
        ...
        const photo_base64 = this.encode_file_attachment(
            args.photo_path,
            base64_photo => {
                const message = email_tmpls.execute('group_new_member', {
                    ...
                    subject:
                        'Welcome to our new member, ' + args.member_first_name,
                });
                // TODO below should be wrapped into send() private method
                const encoded_message = Buffer.from(message)
                    .toString('base64')
