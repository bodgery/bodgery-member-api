...
export let isInteger = (field, allowNull = false) =>
    matchSingleField(field, /^-?[0-9]*$/, 'integer', allowNull);
export let isName = (field, allowNull = false) =>
    matchSingleField(field, /^[A-Za-z0-9]+$/, 'name', allowNull);
export let isWords = (field, allowNull = false) =>
    matchSingleField(field, /^[A-Za-z0-9\s]*$/, 'words', allowNull);
export let isUSPhone = (field, allowNull = false) =>
    matchSingleField(
        field,
...
export let isIdentifier = (field, allowNull = false) =>
    matchSingleField(field, /^[A-Za-z0-9\-\_]*$/, 'identifier', allowNull);
...
export let isUrl = (field, allowNull = false) =>
    matchSingleField(field, urlRegex, 'URL', allowNull);

export let isUSAddress = () =>
    function (params) {
        validate(params, [
            // TODO
...
export let byteLengthLimit = limit =>
    function (param) {
        ...
        if (length > limit) {
            throw new Error(
                'Parameter is ' + length + ' bytes, limit is ' + limit,
            );
        }
    };
 ...
 */
export let isPublicEmail = (field, allowNull = false) =>
    matchSingleField(field, /^[^\s>]+@[^\s>]+\.[^\s>]+$/, 'email', allowNull);

// Taken from:
// https://stackoverflow.com/questions/7905929/how-to-test-valid-uuid-guid
export let isUUID = (field, allowNull = false) =>
    matchSingleField(
        field,
