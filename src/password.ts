...
export interface Crypter {
    crypt(password: string, salt: Buffer, callback: (string) => void): void;
...
export class Checker {
    ...
    constructor(preferred_method: string, db: db.DB) {
        this.db = db;
    ...
    public isMatch(args: {
        ...
        let match_success_callback = (is_matched: boolean) => {
            if (is_matched && stored_crypt_type == this.preferred_method_str) {
                is_match_callback();
            } else if (is_matched) {
    ...
    public addNewUser(
        ...
        this.preferred_method.crypt(pass, salt, crypted_pass => {
            this.db.add_user(
                user,
                crypted_pass,
                salt.toString('hex'),
    ...
    private _parseCryptType(crypt_string: string): Crypter {
        ...
        if (parts.length < 1) {
            throw new Error(
                "Error parsing crypt string ('" + crypt_string + "')",
            );
        }
