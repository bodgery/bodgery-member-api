...
export class MockDB {
    ...
    get_member(
        ...
        if (member_matched != null) {
            ...
            no_member_found_callback(
                new Error(
                    "Could not find match for member ID '" + member_id + "'",
                ),
            );
        }
    ...
    get_member_rfid(
        ...
        let member_id_matched = Object.keys(this.members).find(
            (_, i, obj) => this.members[_].rfid == rfid,
        );
    ...
    get_members(
        ...
        let members = Object.values(this.members).map(_ => _.simple_data);
