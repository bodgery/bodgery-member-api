...
export class PGFixture {
    ...
    public addToDB(db_frontend: pg_db.PG, success_callback: () => void): void {
        ...
        let member_callback = (member_id, area_id) => {
            this.addTool(db, area_id, member_id, (tool1_id, tool1_name) =>
                tool1_callback(tool1_id, tool1_name, area_id, member_id),
            );
        };
        let area_callback = area_id => {
            this.addMember(db, member_id =>
                member_callback(member_id, area_id),
            );
        };
    ...
    private addArea(db, success_callback: (area_id: number) => void): void {
        db.query(
            {
                text: [
                    'INSERT INTO areas (',
            ...
            (err, res) => {
                if (err) {
                    throw new Error(
                        'Error inserting new area into database: ' +
                            err.toString(),
                    );
                } else {
    ...
    private addMember(db, success_callback: (member_id: number) => void): void {
        db.query(
            {
                text: [
                    'INSERT INTO members (',
            ...
            (err, res) => {
                if (err) {
                    throw new Error(
                        'Error inserting new member into database: ' +
                            err.toString(),
                    );
                } else {
                    success_callback(res.rows[0].id);
        ...
        db.query(
            {
                ...
                values: [tool_name, area_id, 'green', owner_id],
            },
            (err, res) => {
                if (err) {
                    throw new Error(
                        'Error inserting new tool into database: ' +
                            err.toString(),
                    );
                } else {
