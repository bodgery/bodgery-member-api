...
function init_server(conf, db, typeorm_connection, logger, wa: wa_api.WA) {
...
function setup_server_routes(conf, db, logger, server, wa: wa_api.WA) {
    ...
    api_v1.put('/member', request_funcs.put_member);
    api_v1.get('/member/:member_id', request_funcs.get_member);
    api_v1
        .route('/member/:member_id/address')
        .put(request_funcs.put_member_address)
        .get(request_funcs.get_member_address);
    api_v1
        .route('/member/:member_id/is_active')
        .put(request_funcs.put_member_is_active)
        .get(request_funcs.get_member_is_active);
    api_v1.put('/member/:member_id/rfid', request_funcs.put_member_rfid);
    api_v1.post(
        '/member/:member_id/send_signup_email',
        request_funcs.post_member_signup_email,
    );
    api_v1.post(
        '/member/:member_id/send_group_signup_email',
        request_funcs.post_group_member_signup_email,
    );
    api_v1.get('/members/pending', request_funcs.get_members_pending);
    api_v1.put(
        '/member/:member_id/wildapricot',
        request_funcs.put_member_wildapricot,
    );
    api_v1.put(
        '/member/:member_id/google_group_signup',
        request_funcs.put_member_google_group,
    );
    api_v1
        .route('/member/:member_id/photo')
        .put(request_funcs.put_member_photo)
        .get(request_funcs.get_member_photo);
    api_v1.get('/rfid/:rfid', request_funcs.get_member_rfid);
    api_v1.get('/rfids', request_funcs.get_rfid_dump);
    api_v1.post(
        '/rfid/log_entry/:rfid/:is_allowed',
    ...
    views.get('/user/is-logged-in', request_funcs.is_user_logged_in);
    views.post('/user/login', request_funcs.login_user);
    ...
    views.get('/member/signup', session_required, request_funcs.member_signup);
    views.get(
        '/members/active',
        session_required,
    ...
    views.post('/user/logout', session_required, request_funcs.logout_user);
    views.get('/rfid/log', session_required, request_funcs.rfid_log);
    views.get('/tokens', session_required, request_funcs.tokens);
    views.post('/tokens/add', session_required, request_funcs.add_token);
    views.post('/tokens/delete', session_required, request_funcs.delete_token);
    ...
    server.use((req, res, next) => {
        logger.error('Route {' + req.method + ' ' + req.path + '} not found');
...
function typeorm_args(conf) {
    return {
        ...
        schema: 'public',
        synchronize: false,
        entities: [path.join(__dirname, 'src/typeorm/entities/*.{js,ts}')],
        ...(conf.db_ssl && {
            extra: {
                ssl: true,
            },
...
export function start(
    ...
    return new Promise((resolve, reject) => {
        createConnection(typeorm_args(conf)).then(typeorm_connection => {
            // Init server
